...
- Added gen_render.sh to combine all schemes into one json file
...
To generate bitmaps and build cursor files, run `generate.sh`. To just generate bitmaps, use cbmp and a config file of your choice from `schemes`. To generate bitmaps for all schemes, run `gen_render_json.sh` then use the `render.json` config file with cbmp. To build cursor files without regenerating bitmaps, run `build.sh`.
